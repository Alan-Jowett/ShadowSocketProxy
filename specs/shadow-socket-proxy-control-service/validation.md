...
| TC-CS-046 | REQ-CS-009 | Valid normalized peer-pin forms versus malformed or wrong pins | Case-insensitive compact, `0x`-prefixed, whitespace-, colon-, and hyphen-separated forms for the matching digest are accepted; malformed values and a different digest are rejected. |
| TC-CS-047 | REQ-CS-009 | Expired, bad-signature, or wrong-key-usage server or client certificate | Rustls/webpki rejects either peer even when the DER pin matches; hostname is not required. |
